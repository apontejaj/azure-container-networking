...
func TestCNSIPAMInvoker_Add_SwiftV2(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "Test happy CNI add with swiftv2 multitenant result with only infraNIC type",
			fields: fields{
				podName:      testPodInfo.PodName,
				podNamespace: testPodInfo.PodNamespace,
				cnsClient: &MockCNSClient{
		...
		{
			name: "Test unhappy CNI add with swiftv2 with DelegatedNIC + BackendNIC interfaces with invalid pnpID",
			fields: fields{
				podName:      testPodInfo.PodName,
				podNamespace: testPodInfo.PodNamespace,
				cnsClient: &MockCNSClient{
