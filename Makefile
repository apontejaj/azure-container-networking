...
test-all: ## run all unit tests.
	@$(eval COVER_FILTER=`go list ./netlink/... | tr '\n' ','`)
	@echo Test coverpkg: $(COVER_FILTER)
	@$(eval COVER_FILES=`go list ./netlink/... | tr '\n' ' '`)
	go test -count 10 -mod=readonly -buildvcs=false -tags "unit" --skip 'TestE2E*' -coverpkg=$(COVER_FILTER) -race -covermode atomic -coverprofile=coverage.out $(COVER_FILES)
