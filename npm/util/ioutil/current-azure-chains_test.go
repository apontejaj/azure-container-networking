...
var listAllCommandStrings = []string{"iptables", "-w", "5", "-W", "10000", "-t", "filter", "-n", "-L"}
...
func TestAllCurrentAzureChains(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			calls: []testutils.TestCmd{
				{Cmd: []string{"iptables", "-w", "5", "-W", "10000", "-t", "filter", "-n", "-L"}, PipedToCommand: true},
				{
					Cmd: []string{"grep", "Chain AZURE-NPM"},
...
`,
		...
		},
		{
			name: "grep failure",
			calls: []testutils.TestCmd{
				{Cmd: []string{"iptables", "-w", "5", "-W", "10000", "-t", "filter", "-n", "-L"}, PipedToCommand: true, HasStartError: true, ExitCode: 1},
				{Cmd: []string{"grep", "Chain AZURE-NPM"}},
			},
			expectedChains: nil,
			wantErr:        true,
