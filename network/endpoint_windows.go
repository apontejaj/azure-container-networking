...
func (nw *network) newEndpointImplHnsV1(epInfo *EndpointInfo) (*endpoint, error) {
	...
	defer func() {
		if err != nil {
			log.Logger.Info("HNSEndpointRequest DELETE id", zap.String("id", hnsResponse.Id))
			hnsResponse, err := Hnsv1.DeleteEndpoint(hnsResponse.Id)
			log.Logger.Error("HNSEndpointRequest DELETE response", zap.Any("hnsResponse", hnsResponse), zap.Error(err))
		}
	}()
	...
	if epInfo.SkipHotAttachEp {
		...
		// Attach the endpoint.
		log.Logger.Info("Attaching endpoint to container", zap.String("id", hnsResponse.Id), zap.String("ContainerID", epInfo.ContainerID))
...
func (nw *network) addIPv6NeighborEntryForGateway(epInfo *EndpointInfo) error {
	...
	if epInfo.IPV6Mode == IPV6Nat {
		...
		if out, err = platform.ExecutePowershellCommand(cmd); err != nil {
			log.Logger.Error("Adding ipv6 gw neigh entry failed", zap.Any("out", out), zap.Error(err))
			return err
		}
	}
...
func (nw *network) configureHcnEndpoint(epInfo *EndpointInfo) (*hcn.HostComputeEndpoint, error) {
	...
	if endpointPolicies, err := policy.GetHcnEndpointPolicies(policy.EndpointPolicy, epInfo.Policies, epInfo.Data, epInfo.EnableSnatForDns, epInfo.EnableMultiTenancy, epInfo.NATInfo); err == nil {
		...
		log.Logger.Error("Failed to get endpoint policies due to", zap.Error(err))
		return nil, err
	}
...
func (nw *network) deleteHostNCApipaEndpoint(networkContainerID string) error {
	...
	log.Logger.Info("Deleting HostNCApipaEndpoint for NC", zap.String("endpointName", endpointName), zap.String("networkContainerID", networkContainerID))
	...
	if err != nil {
		...
		log.Logger.Error("Delete called on the Endpoint which doesn't exist. Error:", zap.String("endpointName", endpointName), zap.Error(err))
		return nil
	}
	...
	log.Logger.Info("Successfully deleted HostNCApipa endpoint", zap.Any("endpoint", endpoint))
...
func (nw *network) createHostNCApipaEndpoint(cli apipaClient, epInfo *EndpointInfo) error {
	...
	log.Logger.Info("Creating HostNCApipaEndpoint for host container connectivity for NC",
...
func (nw *network) newEndpointImplHnsV2(cli apipaClient, epInfo *EndpointInfo) (*endpoint, error) {
	...
	if err != nil {
		log.Logger.Error("Failed to configure hcn endpoint due to", zap.Error(err))
		return nil, err
	}

	// Create the HCN endpoint.
	log.Logger.Info("Creating hcn endpoint computenetwork", zap.String("hcnEndpointName", hcnEndpoint.Name), zap.Any("HostComputeNetwork", hcnEndpoint.HostComputeNetwork))
	...
	log.Logger.Info("Successfully created hcn endpoint with response", zap.Any("hnsResponse", hnsResponse))

	defer func() {
		if err != nil {
			log.Logger.Info("Deleting hcn endpoint with id", zap.String("id", hnsResponse.Id))
			err = Hnsv2.DeleteEndpoint(hnsResponse)
			log.Logger.Error("Completed hcn endpoint deletion for id with error", zap.String("id", hnsResponse.Id), zap.Error(err))
		}
	}()
...
func (nw *network) deleteEndpointImplHnsV1(ep *endpoint) error {
	log.Logger.Info("HNSEndpointRequest DELETE id", zap.String("id", ep.HnsId))
	hnsResponse, err := Hnsv1.DeleteEndpoint(ep.HnsId)
	log.Logger.Info("HNSEndpointRequest DELETE response err", zap.Any("hnsResponse", hnsResponse), zap.Error(err))
	...
	if err != nil {
		if strings.Contains(strings.ToLower(err.Error()), "not found") {
			log.Logger.Info("HNS endpoint id not found", zap.String("id", ep.HnsId))
			return nil
		}
	}
...
func (nw *network) deleteEndpointImplHnsV2(ep *endpoint) error {
	...
	log.Logger.Info("Deleting hcn endpoint with id", zap.String("HnsId", ep.HnsId))
	...
	if err != nil {
		...
		log.Logger.Error("Delete called on the Endpoint which doesn't exist. Error:", zap.String("HnsId", ep.HnsId), zap.Error(err))
		return nil
	}
	...
	log.Logger.Info("Successfully deleted hcn endpoint with id", zap.String("HnsId", ep.HnsId))
