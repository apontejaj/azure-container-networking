...
func NewInfraVnetClient(hostIfName, contIfName string, nl netlink.NetlinkInterface, plc platform.ExecClient) OVSInfraVnetClient {
	...
	log.NetLogger.Info("Initialize new infravnet client", zap.Any("infraVnetClient", infraVnetClient))
...
func (client *OVSInfraVnetClient) CreateInfraVnetEndpoint(bridgeName string) error {
	...
	if err := epc.CreateEndpoint(client.hostInfraVethName, client.ContainerInfraVethName, nil); err != nil {
		log.NetLogger.Error("Creating infraep failed with", zap.Error(err))
		return err
	}

	log.NetLogger.Info("Adding port master", zap.String("hostInfraVethName", client.hostInfraVethName), zap.String("bridgeName", bridgeName))
	if err := ovs.AddPortOnOVSBridge(client.hostInfraVethName, bridgeName, 0); err != nil {
		log.NetLogger.Error("Adding infraveth to ovsbr failed with", zap.Error(err))
		return err
	}

	infraContainerIf, err := net.InterfaceByName(client.ContainerInfraVethName)
	if err != nil {
		log.NetLogger.Error("InterfaceByName returns error for ifname with", zap.String("ContainerInfraVethName", client.ContainerInfraVethName),
			zap.Error(err))
		return err
	}
...
func (client *OVSInfraVnetClient) CreateInfraVnetRules(
	...
	if err := ovs.AddIPSnatRule(bridgeName, infraIP.IP, 0, infraContainerPort, hostPrimaryMac, hostPort); err != nil {
		log.NetLogger.Error("[ovs] AddIpSnatRule failed with", zap.Error(err))
		return err
	}
	...
	if err := ovs.AddMacDnatRule(bridgeName, hostPort, infraIP.IP, client.containerInfraMac, 0, infraContainerPort); err != nil {
		log.NetLogger.Error("[ovs] AddMacDnatRule failed with", zap.Error(err))
		return err
	}
...
func (client *OVSInfraVnetClient) MoveInfraEndpointToContainerNS(netnsPath string, nsID uintptr) error {
	log.NetLogger.Info("[ovs] Setting link netns", zap.String("ContainerInfraVethName", client.ContainerInfraVethName),
...
func (client *OVSInfraVnetClient) ConfigureInfraVnetContainerInterface(infraIP net.IPNet) error {
	log.NetLogger.Info("[ovs] Adding IP address to link", zap.String("IP", infraIP.String()), zap.String("ContainerInfraVethName", client.ContainerInfraVethName))
...
func (client *OVSInfraVnetClient) DeleteInfraVnetRules(
	...
	log.NetLogger.Info("[ovs] Deleting MAC DNAT rule for infravnet IP address", zap.String("IP", infraIP.IP.String()))
	ovs.DeleteMacDnatRule(bridgeName, hostPort, infraIP.IP, 0)

	log.NetLogger.Info("[ovs] Get ovs port for infravnet interface", zap.String("hostInfraVethName", client.hostInfraVethName))
	infraContainerPort, err := ovs.GetOVSPortNumber(client.hostInfraVethName)
	if err != nil {
		log.NetLogger.Error("[ovs] Get infravnet portnum failed with", zap.Error(err))
	}

	log.NetLogger.Info("Deleting IP SNAT for infravnet port", zap.String("infraContainerPort", infraContainerPort))
	ovs.DeleteIPSnatRule(bridgeName, infraContainerPort)

	log.NetLogger.Info("[ovs] Deleting infravnet interface", zap.String("hostInfraVethName", client.hostInfraVethName), zap.String("bridgeName", bridgeName))
	if err := ovs.DeletePortFromOVS(bridgeName, client.hostInfraVethName); err != nil {
		log.NetLogger.Error("[ovs] Deletion of infravnet interface", zap.String("hostInfraVethName", client.hostInfraVethName), zap.String("bridgeName", bridgeName))
	}
}

func (client *OVSInfraVnetClient) DeleteInfraVnetEndpoint() error {
	log.NetLogger.Info("[ovs] Deleting Infra veth pair", zap.String("hostInfraVethName", client.hostInfraVethName))
	err := client.netlink.DeleteLink(client.hostInfraVethName)
	if err != nil {
		log.NetLogger.Error("[ovs] Failed to delete veth pair", zap.String("hostInfraVethName", client.hostInfraVethName),
			zap.Error(err))
		return newErrorOVSInfraVnetClient(err.Error())
	}
