...
func (client *OVSEndpointClient) AddSnatEndpoint() error {
	if client.isSnatEnabled() {
		...
		log.Logger.Info("Drop ARP for snat bridge ip", zap.String("SnatBridgeIP", snatClient.SnatBridgeIP))
		...
		if err == nil {
			log.Logger.Info("Azure snat veth already exists")
			return nil
		}
		...
		if err != nil {
			log.Logger.Error("Failed to create veth pair", zap.Any("error:", err), zap.String("component", "net"))
